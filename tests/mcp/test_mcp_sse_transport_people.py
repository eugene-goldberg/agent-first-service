"""End-to-end SSE-transport test for the People MCP server.

Launches ``python -m services.people.mcp_main --sse --port 9092``. Uses the
``mcp`` SSE client to repeat the ``tools/list`` + ``tools/call`` assertions.
Uses port 9092 to avoid clashing with:

* 8002 — People HTTP service
* 9002 — People MCP SSE port used in production / dev runs
...
async def test_sse_transport_list_and_call_people(tmp_path) -> None:
    """Full MCP SSE round-trip: list_tools + call_tool create/list people."""
        ...
        async def _run_session() -> None:
            async with sse_client(sse_url) as (read, write):
                async with ClientSession(read, write) as session:
                    ...
                    assert create_envelope["data"]["name"] == "SSE Person"

                    # 4c) tools/call list_people ------------------------
                    ...
                    assert "SSE Person" in names_seen

...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Projects MCP server (stdio by default, --sse for HTTP/SSE)"
    )
    parser.add_argument(
        "--sqlite-path",
        type=Path,
        default=Path("projects.db"),
        help="Path to the SQLite database backing the Projects service.",
    )
    parser.add_argument(
        "--sse",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=9001,
        help="Port to bind when running in --sse mode (default: 9001).",
    )
    parser.add_argument(
        "--host",
        type=str,
...
async def _run_stdio(sqlite_path: Path) -> None:
    ...
    server, _adapter = build_projects_mcp_server(sqlite_path=sqlite_path)

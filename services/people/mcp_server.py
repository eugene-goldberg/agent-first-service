"""Build an MCP server for the People service.
...
bound to a real People FastAPI app. Transport selection (stdio / SSE) is the
responsibility of the entrypoint module (:mod:`services.people.mcp_main`).
"""
...
def build_people_mcp_server(
    *, sqlite_path: Path
) -> tuple[Server, CatalogBackedMCPServer]:
    """Construct an ``mcp.server.Server`` wired to a real People FastAPI app.

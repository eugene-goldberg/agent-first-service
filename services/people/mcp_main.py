"""MCP server entrypoint for the People service (stdio + SSE transports).

Run in stdio mode (default — Claude Desktop, local subprocess drivers)::

    python -m services.people.mcp_main --sqlite-path /path/to/people.db

Run in SSE mode (orchestrator integration, long-running network transport)::

    python -m services.people.mcp_main --sse --port 9002 \
        --sqlite-path /path/to/people.db
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="People MCP server (stdio by default, --sse for HTTP/SSE)"
    )
    parser.add_argument(
        "--sqlite-path",
        type=Path,
        default=Path("people.db"),
        help="Path to the SQLite database backing the People service.",
    )
    parser.add_argument(
        "--sse",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=9002,
        help="Port to bind when running in --sse mode (default: 9002).",
    )
    parser.add_argument(
        "--host",
        type=str,
...
async def _run_stdio(sqlite_path: Path) -> None:
    ...
    from services.people.mcp_server import build_people_mcp_server
    ...
    server, _adapter = build_people_mcp_server(sqlite_path=sqlite_path)
